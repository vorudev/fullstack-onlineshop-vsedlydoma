...
<div className="flex flex-col max-w-[250px]"><h3 className="text-lg xl:text-xl font-semibold ">{session.data?.user ? "Привет, " + session?.data?.user?.name + "!": "Личный кабинет" }</h3>
    <p className="text-gray-600 text-sm xl:text-base">Отслеживайте свои заказы в личном кабинете</p>
    </div> 
    <div className="flex flex-row gap-2">
       {session.data?.user ? <Link href="/profile" className="bg-white rounded-lg hover:bg-gray-100 transition-colors duration-200 text-sm border border-gray-300 py-2 px-3 xl:text-base">Мои заказы</Link> : <Link href="/signin" className="bg-white rounded-lg hover:bg-gray-100 transition-colors duration-200 text-sm border border-gray-300 py-2 px-3 xl:text-base">Войти</Link>} 
