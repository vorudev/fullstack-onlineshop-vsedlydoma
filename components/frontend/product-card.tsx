...
export default async function ProductCard( product: ProductCardProps) {
  const [{averageRating, reviewCount}, image] = await Promise.all([getAverageRatingByProductId(product.id), getFeaturedImage(product.id)]);
...
function getReviewText(count: number): string {
  ...
  return `${count} ${word}`;
    ...
    return ( 
        <div className="bg-white rounded-2xl  hover:shadow-xl transition-all duration-300 overflow-hidden  group " key={product.id}> 
