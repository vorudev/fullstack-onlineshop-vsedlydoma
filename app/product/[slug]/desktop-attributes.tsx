...
export default function DesktopAtributes({productDetails}: ProductUnited) {
  ...
  <div className="grid grid-cols-2 gap-x-8">
    ...
    <div>
      {productDetails?.attributes.slice(Math.ceil(productDetails?.attributes.length / 2)).map((attr, index) => (
        <div
          key={attr.id || attr.slug || (index + Math.ceil(productDetails?.attributes.length / 2))}
          className="grid grid-cols-[minmax(200px,1fr)_minmax(150px,1fr)] text-[14px] gap-10 py-1"
        >
          <div className="text-gray-500 font-medium break-words">
            {attr.name || 'Не указано'}
